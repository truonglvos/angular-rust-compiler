//! IR Enums
//!
//! Corresponds to packages/compiler/src/template/pipeline/ir/src/enums.ts

use bitflags::bitflags;

/// Compatibility mode for template compilation
/// Whether to compile in compatibility mode. In compatibility mode, the template pipeline will
...
/// OpKind enum - distinguishes different kinds of IR operations
...
pub enum OpKind {
    ...
    /// Configure a content projection definition for the view.
    ProjectionDef,
    ...
    /// An operation to set up a corresponding `Control` operation.
...
bitflags! {
    /// Flags that describe variable behavior.
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
bitflags! {
    ...
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
    pub struct TDeferDetailsFlags: u32 {
        const DEFAULT = 0;

        /// Whether or not the defer block has hydrate triggers.
        const HAS_HYDRATE_TRIGGERS = 1 << 0;
    }
}
