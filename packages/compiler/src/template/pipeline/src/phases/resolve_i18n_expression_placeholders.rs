...
use crate::template::pipeline::ir::enums::{
    I18nExpressionFor, I18nParamResolutionTime, I18nParamValueFlags, OpKind,
};
use crate::template::pipeline::ir::ops::create::I18nParamValue;
use crate::template::pipeline::ir::ops::create::{I18nContextOp, I18nStartOp, IcuPlaceholderOp};
...
pub fn resolve_i18n_expression_placeholders(job: &mut dyn CompilationJob) {
    ...
    let mut sub_template_indices: std::collections::HashMap<ir::XrefId, Option<usize>> =
    ...
    let mut expression_indices: std::collections::HashMap<ir::XrefId, usize> =
        std::collections::HashMap::new();
    ...
    for op in component_job.root.update.iter() {
        if op.kind() == OpKind::I18nExpression {
            unsafe {
                ...
                let sub_template_index = sub_template_indices
                    .get(&expr_op.i18n_owner)
                    .copied()
                    .flatten();
    ...
    for (expr_op, value, _) in root_updates {
        update_placeholder(&expr_op, value, &mut i18n_contexts, &mut component_job.root);
    }

    // Process all view units - collect first
    let mut view_updates: Vec<(ir::XrefId, I18nExpressionOp, I18nParamValue, ir::XrefId)> =
        Vec::new();
    ...
    for (view_key, expr_op, value, _) in view_updates {
        if let Some(unit) = component_job.views.get_mut(&view_key) {
            update_placeholder(&expr_op, value, &mut i18n_contexts, unit);
        }
    }
}
...
fn update_placeholder(
    ...
    if let Some(icu_placeholder_xref) = op.icu_placeholder {
        ...
        for create_op in unit.create.iter_mut() {
            if create_op.kind() == OpKind::IcuPlaceholder
                && create_op.xref() == icu_placeholder_xref
            {
                unsafe {
