...
use crate::template::pipeline::ir::ops::create::{ElementStartOp, ElementEndOp, ContainerStartOp, ContainerEndOp};
        ...
        if op.kind() == OpKind::ElementStart {
            unsafe {
                ...
                if elem_start.base.tag.as_deref() == Some(CONTAINER_TAG) {
                    ...
                    let mut container_start = ContainerStartOp::new(xref, start_source_span, whole_source_span);
                    ...
                    if let Some(source_span) = elem_end.source_span.clone() {
                        ...
                        let default_source_span = crate::parse_util::ParseSourceSpan::new(
                            crate::parse_util::ParseLocation::new(
                                crate::parse_util::ParseSourceFile::new("".to_string(), "".to_string()),
                                0, 0, 0
                            ),
                            crate::parse_util::ParseLocation::new(
                                crate::parse_util::ParseSourceFile::new("".to_string(), "".to_string()),
