...
use crate::template::pipeline::ir::ops::update::{AttributeOp, StylePropOp, StyleMapOp, ClassMapOp, BindingExpression};
                ...
                OpKind::Attribute => {
                    ...
                    if let BindingExpression::Interpolation(ref interp) = attr_op.expression {
                        if interp.strings.len() == 2 
                            ...
                            attr_op.expression = BindingExpression::Expression(
                                interp.expressions[0].clone()
                            );
                        }
                    }
                }
                OpKind::StyleProp => {
                    ...
                    if let BindingExpression::Interpolation(ref interp) = style_prop_op.expression {
                        if interp.strings.len() == 2 
                            ...
                            && interp.expressions.len() == 1 {
                            // Collapse: replace interpolation with first expression
                            style_prop_op.expression = BindingExpression::Expression(
                                interp.expressions[0].clone()
                            );
                        }
                    }
                ...
                OpKind::StyleMap => {
                    ...
                    if let BindingExpression::Interpolation(ref interp) = style_map_op.expression {
                        if interp.strings.len() == 2 
                            ...
                            && interp.expressions.len() == 1 {
                            // Collapse: replace interpolation with first expression
                            style_map_op.expression = BindingExpression::Expression(
                                interp.expressions[0].clone()
                            );
                        }
                    }
                ...
                OpKind::ClassMap => {
                    ...
                    if let BindingExpression::Interpolation(ref interp) = class_map_op.expression {
                        if interp.strings.len() == 2 
                            ...
                            && interp.expressions.len() == 1 {
                            // Collapse: replace interpolation with first expression
                            class_map_op.expression = BindingExpression::Expression(
                                interp.expressions[0].clone()
                            );
                        }
                    }
