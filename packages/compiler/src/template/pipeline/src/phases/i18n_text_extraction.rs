...
use crate::template::pipeline::ir::ops::create::{I18nStartOp, IcuStartOp, TextOp, IcuPlaceholderOp, create_icu_placeholder_op};
use crate::template::pipeline::ir::ops::update::{InterpolateTextOp, create_i18n_expression_op};
...
fn process_unit(
    ...
    let mut text_node_i18n_blocks: std::collections::HashMap<ir::XrefId, usize> = std::collections::HashMap::new();
    let mut text_node_icus: std::collections::HashMap<ir::XrefId, Option<usize>> = std::collections::HashMap::new();
    let mut icu_placeholder_by_text: std::collections::HashMap<ir::XrefId, ir::XrefId> = std::collections::HashMap::new();
    ...
    let mut interpolate_indices_to_replace: Vec<(usize, Vec<Box<dyn ir::UpdateOp + Send + Sync>>)> = Vec::new();
        ...
        if op.kind() == OpKind::InterpolateText {
            unsafe {
                ...
                if let Some(&i18n_idx) = text_node_i18n_blocks.get(&interpolate.target) {
                    ...
                    let icu_placeholder_xref = icu_placeholder_by_text.get(&interpolate.target).copied();
                    ...
                    for (i, expr) in interpolate.interpolation.expressions.iter().enumerate() {
                        ...
                        let i18n_expr_op = create_i18n_expression_op(
                            ...
                            String::new(),
                            expr.source_span().cloned().unwrap_or_else(|| interpolate.source_span.clone()),
                        );
                    ...
                    if let Some(icu_placeholder_xref) = icu_placeholder_xref {
                        icu_placeholder_updates.push((icu_placeholder_xref, interpolate.interpolation.strings.clone()));
                    }
                }
            }
        }
