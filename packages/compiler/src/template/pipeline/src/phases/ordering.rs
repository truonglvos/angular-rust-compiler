...
use crate::template::pipeline::ir::ops::update::{
    AttributeOp, BindingExpression, PropertyOp, 
};
...
use crate::template::pipeline::src::compilation::{
    ...
    ViewCompilationUnit, HostBindingCompilationUnit,
};
...
const CREATE_ORDERING: &[Rule<dyn ir::CreateOp + Send + Sync>] = &[
    // {test: listener...}
    Rule { test: basic_listener_kind_test, transform: None },
];
...
fn order_within<T: 'static + ?Sized>(
    ops: OpList<Box<T>>, 
    ordering: Vec<Rule<T>>,
) -> OpList<Box<T>> 
where T: ir::Op
{
    ...
    for op in ops.into_vec() {
        ...
        if !is_handled || (first_target_in_group.is_some() && current_target.is_some() && current_target != first_target_in_group) {
            // Flush
            if !buffer.is_empty() {
...
fn get_dependency_target<T: ir::Op + ?Sized>(op: &T) -> Option<ir::XrefId> {
    ...
    // Manual check:
    match op.kind() {
        OpKind::Property | OpKind::Attribute | OpKind::StyleProp | OpKind::ClassProp | OpKind::TwoWayProperty | OpKind::Binding => {
             ...
             // Workaround:
             if let Some(prop) = op.as_any().downcast_ref::<PropertyOp>() { Some(prop.target) }
             else if let Some(attr) = op.as_any().downcast_ref::<AttributeOp>() { Some(attr.target) }
             // ... add others
             else { None }
        }
        _ => None
...
fn get_create_rules() -> Vec<Rule<dyn ir::CreateOp + Send + Sync>> {
    vec![
        // simplified
        Rule { test: |op| op.kind() == OpKind::Listener, transform: None }, 
    ]
}

fn get_update_rules() -> Vec<Rule<dyn ir::UpdateOp + Send + Sync>> {
    vec![
       Rule { test: |op| op.kind() == OpKind::StyleMap, transform: Some(keep_last) },
       Rule { test: |op| op.kind() == OpKind::ClassMap, transform: Some(keep_last) },
       Rule { test: |op| op.kind() == OpKind::StyleProp, transform: None },
       Rule { test: |op| op.kind() == OpKind::ClassProp, transform: None },
       Rule { test: |op| op.kind() == OpKind::Attribute && check_interpolation(op), transform: None },
       Rule { test: |op| op.kind() == OpKind::Property && check_interpolation(op), transform: None },
       Rule { test: non_interpolation_property_kind_test, transform: None },
       Rule { test: |op| op.kind() == OpKind::Attribute && !check_interpolation(op), transform: None },
    ]
}
