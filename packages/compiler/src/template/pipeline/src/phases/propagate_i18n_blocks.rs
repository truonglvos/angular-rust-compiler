...
use crate::template::pipeline::ir::ops::create::{
    create_i18n_end_op, create_i18n_start_op, ConditionalBranchCreateOp, ConditionalCreateOp,
    I18nStartOp, ProjectionOp, RepeaterCreateOp, TemplateOp,
};
use crate::template::pipeline::src::compilation::{
    CompilationJob, ComponentCompilationJob, ViewCompilationUnit,
};
        ...
        match op.kind() {
            OpKind::I18nStart => {
                if current_sub_template_index == 0 {
                    ...
                    unsafe {
                        let op_mut_ptr =
                            unit.create.get_mut(idx).unwrap().as_mut() as *mut dyn ir::CreateOp;
            ...
            OpKind::ConditionalCreate | OpKind::ConditionalBranchCreate | OpKind::Template => unsafe {
            ...
            OpKind::RepeaterCreate => {
                unsafe {
                    ...
                    if let Some(for_view) = job.views.get_mut(&view_xref) {
                        let parent_i18n = if let Some(i18n_idx) = i18n_block {
                            let op_ptr = unit.create.get(i18n_idx).unwrap().as_ref()
                                as *const dyn ir::CreateOp;
                            let i18n_ptr = op_ptr as *const I18nStartOp;
                            Some(&*i18n_ptr)
                        } else {
                            None
                ...
                if let Some(fallback_view_xref) = projection.fallback_view {
                    if let Some(fallback_view) = job.views.get_mut(&fallback_view_xref) {
                        ...
                        current_sub_template_index = propagate_i18n_blocks_for_view(
                            ...
                            parent_i18n,
                            projection
                                .fallback_view_i18n_placeholder
                                .as_ref()
                                .map(|p| p as &dyn std::any::Any),
                            current_sub_template_index,
                            job_ptr,
