...
mod tests {
    ...
    use angular_compiler::ml_parser::html_whitespaces::{remove_whitespaces, PRESERVE_WS_ATTR_NAME};
    ...
    fn should_replace_multiple_whitespaces_with_one_space() {
        ...
        assert_eq!(result1, vec![
            vec!["Text".to_string(), " foo ".to_string(), "0".to_string()],
        ]);
    ...
    fn should_preserve_whitespaces_inside_pre_elements() {
        let result = parse_and_remove_ws("<pre><strong>foo</strong>\n<strong>bar</strong></pre>", None);
    ...
    fn should_skip_whitespace_trimming_in_textarea() {
        ...
        assert_eq!(result, vec![
            vec!["Element".to_string(), "textarea".to_string(), "0".to_string()],
            vec!["Text".to_string(), "foo\n\n  bar".to_string(), "1".to_string()],
        ]);
    }
