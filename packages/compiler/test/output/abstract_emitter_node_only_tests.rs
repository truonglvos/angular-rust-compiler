...
fn create_source_span(file: &ParseSourceFile, idx: usize) -> MockSourceSpan {
    ...
    MockSourceSpan(ParseSourceSpan { start, end, details: None })
}
...
mod tests {
    ...
    fn expect_map(
        ...
        assert_eq!(orig_pos.line, src_line.map(|l| l + 1), "Line mismatch (expected 1-based)"); // original_position_for returns 1-based line
        assert_eq!(orig_pos.column, src_col, "Column mismatch");
    }

    #[test]
    fn should_add_source_files_to_the_source_map() {
        let file_a = ParseSourceFile { content: "a0a1a2a3a4a5a6a7a8a9".to_string(), url: "a.js".to_string() };
        let file_b = ParseSourceFile { content: "b0b1b2b3b4b5b6b7b8b9".to_string(), url: "b.js".to_string() };
        ...
        let content_a = sm.sources_content.iter().find(|c| c.as_ref().map(|s| s == &file_a.content).unwrap_or(false));
        let content_b = sm.sources_content.iter().find(|c| c.as_ref().map(|s| s == &file_b.content).unwrap_or(false));
