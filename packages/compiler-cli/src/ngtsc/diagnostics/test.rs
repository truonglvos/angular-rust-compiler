...
use crate::ngtsc::diagnostics::{
    ng_error_code, replace_ts_with_ng_in_errors, ErrorCode, FatalDiagnosticError, Node,
};
use ts::{LanguageVariant, NodeFlags, ScriptTarget, SourceFile, SyntaxKind};
...
impl Node for MockNode {
    ...
    fn get_source_file(&self) -> Option<&dyn SourceFile> {
        self.source_file
            .as_ref()
            .map(|sf| sf.as_ref() as &dyn SourceFile)
    }
    fn parent(&self) -> Option<&dyn Node> {
        None
    }
