use crate::ngtsc::metadata::{extract_directive_metadata, DecoratorMetadata, DirectiveMetadata, ModuleMetadataReader};
        ...
        for decorator in converted_decorators {
            if decorator.name == "Component" {
                ...
                if let Some(metadata) =
                    extract_directive_metadata(node, &decorator, true, std::path::Path::new(""), &empty_imports)
                {
    ...
    pub fn compile_ivy(&self, analysis: &DirectiveMetadata<'static>) -> Vec<CompileResult> {
        ...
        if let Some(imports) = &dir.imports {
            ...
            for import_ref in imports {
                ...
                let import_name = import_ref.debug_name().to_string();
                let module_path = import_ref.best_guess_owning_module.as_ref().map(|m| m.specifier.clone());
                ...
                if let Some(path) = &module_path {
                    ...
                    if let Some(dynamic_deps) = metadata_reader.read_metadata(path) {
                        ...
                        for meta in &dynamic_deps {
                            ...
                            if let Some(name) = meta_name {
                                if name == &import_name || (name == "NgForOf" && import_name == "NgFor") || (name == "NgIf" && import_name == "NgIf") {
                                    matched_specific_symbol = true;
                                    
                                    let key = match &meta {
                                        R3TemplateDependencyMetadata::Directive(d) => format!("dir:{}", d.selector),
                                        R3TemplateDependencyMetadata::Pipe(p) => format!("pipe:{}", p.name),
                                        R3TemplateDependencyMetadata::NgModule(_) => format!("module:{}", import_name),
                                    };
                        ...
                        if !matched_specific_symbol {
                            ...
                            for mut meta in dynamic_deps {
                                let key = match &meta {
                                    R3TemplateDependencyMetadata::Directive(d) => format!("dir:{}", d.selector),
                                    R3TemplateDependencyMetadata::Pipe(p) => format!("pipe:{}", p.name),
                                    R3TemplateDependencyMetadata::NgModule(_) => format!("module:unknown"), // Should not happen for exports usually
                                };
                    ...
                    if let Some(source_file) = &dir.source_file {
                        if let Some(local_details) = metadata_reader.extract_ts_metadata(source_file) {
        ...
        let mut r3_metadata = R3ComponentMetadata {
            directive: R3DirectiveMetadata {
                ...
                queries: dir.queries.iter().map(|q| {
                     angular_compiler::render3::view::api::R3QueryMetadata {
                        property_name: q.property_name.clone(),
                        first: q.first,
                        predicate: angular_compiler::render3::view::api::R3QueryPredicate::Selectors(vec![q.selector.clone()]),
                ...
                view_queries: dir.view_queries.iter().map(|vq| {
                    angular_compiler::render3::view::api::R3QueryMetadata {
                        property_name: vq.property_name.clone(),
                        first: vq.first,
                        predicate: angular_compiler::render3::view::api::R3QueryPredicate::Selectors(vec![vq.selector.clone()]),
