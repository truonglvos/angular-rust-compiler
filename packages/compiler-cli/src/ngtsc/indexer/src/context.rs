...
impl IndexingContext {
    ...
    pub fn add_component(&mut self, file: &str, component: IndexedComponent) {
        self.components.entry(file.to_string()).or_default().push(component);
    }
    
    pub fn add_directive(&mut self, file: &str, directive: IndexedDirective) {
        self.directives.entry(file.to_string()).or_default().push(directive);
    }
...
impl Indexer {
    pub fn new() -> Self {
        Self { context: IndexingContext::new() }
    }
