        ...
        for (i, segment) in segments.iter().enumerate() {
            match current.get(segment) {
                Some(Entity::Folder(map)) => {
                    if i == segments.len() - 1 {
                        return Some(Entity::Folder(map.clone()));
                    }
                    current = map;
                }
                Some(Entity::File(content)) => {
                    if i == segments.len() - 1 {
                        return Some(Entity::File(content.clone()));
                    }
                    return None; // File mid-path
                }
                Some(Entity::SymLink(target)) => {
                    if i == segments.len() - 1 {
                        return Some(Entity::SymLink(target.clone()));
                    }
                    return None;
                }
                None => return None,
...
impl FileSystem for MockFileSystem {
    fn write_file(
        ...
        if segments.is_empty() {
            return Err(io::Error::new(io::ErrorKind::Other, "Cannot write to root"));
        }
        ...
        for segment in dir_segments {
            if !current.contains_key(segment) {
                return Err(io::Error::new(
                    ...
                    format!("Directory {} not found", segment),
                ));
            }
    ...
    fn remove_file(&self, path: &AbsoluteFsPath) -> io::Result<()> {
        ...
        let segments = self.strategy.split_path(path.as_str());
        if segments.is_empty() {
            return Err(io::Error::new(io::ErrorKind::Other, "Root"));
        }
...
impl ReadonlyFileSystem for MockFileSystem {
    fn is_case_sensitive(&self) -> bool {
        self.strategy.is_case_sensitive()
    }
    ...
    fn lstat(&self, path: &AbsoluteFsPath) -> io::Result<FileStats> {
        match self.get_entity(path) {
            Some(Entity::File(_)) => Ok(FileStats {
                is_file: true,
                is_directory: false,
                is_symbolic_link: false,
            }),
            Some(Entity::Folder(_)) => Ok(FileStats {
                is_file: false,
                is_directory: true,
                is_symbolic_link: false,
            }),
            Some(Entity::SymLink(_)) => Ok(FileStats {
                is_file: false,
                is_directory: false,
...
impl PathManipulation for MockFileSystem {
    fn dirname(&self, file: &str) -> String {
        self.strategy.dirname(file)
    }
    fn join(&self, base_path: &str, paths: &[&str]) -> String {
        self.strategy.join(base_path, paths)
    }
    ...
    fn basename(&self, path: &str, ext: Option<&str>) -> PathSegment {
        PathSegment::new(self.strategy.basename(path, ext))
    }
    ...
    fn is_root(&self, path: &AbsoluteFsPath) -> bool {
        self.strategy.is_root(path.as_str())
    }
    ...
    fn normalize(&self, path: &str) -> String {
        self.strategy.normalize(path)
    }
    fn relative(&self, from: &str, to: &str) -> String {
        self.strategy.relative(from, to)
    }
