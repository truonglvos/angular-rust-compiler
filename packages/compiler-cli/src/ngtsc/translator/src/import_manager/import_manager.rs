...
use crate::ngtsc::translator::src::import_manager::check_unique_identifier_name::{IdentifierScope, UniqueIdentifierGenerator};
use crate::ngtsc::translator::src::import_manager::reuse_generated_imports::{ReuseGeneratedImportsTracker, attempt_to_reuse_generated_imports, capture_generated_import};
use crate::ngtsc::translator::src::import_manager::reuse_source_file_imports::{ReuseExistingSourceFileImportsTracker, SourceFileImports, attempt_to_reuse_existing_source_file_imports};
    ...
    A::Expression: Clone
{
    ...
    fn get_new_imports_tracker_for_file(&mut self, file: &TFile) -> &mut NewImportsForFile {
        self.new_imports.entry(file.clone()).or_insert(NewImportsForFile {
        ...
        for (file, tracker) in &self.new_imports {
             ...
             for (module, specifiers) in &tracker.named_imports {
                  ...
                  if !specifiers.is_empty() {
                      ...
                      let specifiers_cloned = specifiers.iter().map(|s| ImportSpecifierInternal {
                          name: s.name.clone(),
                          alias: s.alias.clone(),
                      }).collect();
    ...
    A::Expression: Clone
{
    fn add_import(&mut self, request: ImportRequest<TFile>) -> A::Expression {
        // Reuse generated
        if let Some(reused) = attempt_to_reuse_generated_imports(&self.reuse_generated_imports_tracker, &request) {
        ...
        if !self.config.disable_original_source_file_reuse {
             if let Some(reused) = attempt_to_reuse_existing_source_file_imports(&mut self.reuse_source_file_imports_tracker, &request.requested_file, &request, self.ast_factory) {
                 return reused;
             }
        }
        ...
        // Namespace Import
        if request.export_symbol_name.is_none() || self.config.force_generate_namespaces_for_new_imports {
             // Logic to generate namespace import
             let mut ns_name = format!("{}{}", self.config.namespace_import_prefix, self.next_unique_index);
             self.next_unique_index += 1;
             
             // Check unique
             if let Some(unique) = self.unique_id_generator.generate_unique_identifier(&file, &ns_name) {
                 ns_name = unique;
             }
             
             // Store in tracker
             let tracker = self.new_imports.entry(file.clone()).or_insert(NewImportsForFile {
                namespace_imports: HashMap::new(),
                named_imports: HashMap::new(),
                side_effect_imports: HashSet::new(),
             });
             tracker.namespace_imports.insert(request.export_module_specifier.clone(), ns_name.clone());
             
             let ns_expr = self.ast_factory.create_identifier(&ns_name);
             capture_generated_import(&request, &mut self.reuse_generated_imports_tracker, ns_expr.clone());
        ...
        let unique_name = if let Some(alias) = &request.unsafe_alias_override {
             ...
             self.unique_id_generator.generate_unique_identifier(&file, symbol_name).unwrap_or(symbol_name.clone())
        };

        let needs_alias = &unique_name != symbol_name || request.unsafe_alias_override.is_some();
        let specifier_alias = if needs_alias { Some(unique_name.clone()) } else { None };
        ...
        let exports = tracker.named_imports.entry(request.export_module_specifier.clone()).or_insert(Vec::new());
        ...
        capture_generated_import(&request, &mut self.reuse_generated_imports_tracker, expr.clone());
        expr
    }
}
