...
mod tests {
    ...
    use crate::ngtsc::metadata::{DecoratorMetadata, MetadataReader, OxcMetadataReader};
    ...
    "#;
        ...
        if let DecoratorMetadata::Directive(meta) = &directives[0] {
            ...
            assert_eq!(
                meta.component
                    .as_ref()
                    .and_then(|c| c.template_url.as_ref()),
                Some(&"./assets.component.html".to_string())
            );
            assert_eq!(
                meta.component.as_ref().and_then(|c| c.styles.as_ref()),
                Some(&vec!["div { color: red; }".to_string()])
            );
            assert_eq!(
                meta.component.as_ref().and_then(|c| c.style_urls.as_ref()),
                Some(&vec![
                    "./assets.component.css".to_string(),
                    "./other.css".to_string()
                ])
            );
        } else {
