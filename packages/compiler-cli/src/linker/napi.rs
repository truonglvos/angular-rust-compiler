...
pub fn link_file(source_code: String, filename: String) -> Result<String> {
    ...
    impl<'a> LinkerVisitor<'a> {
        fn new(source_code: &'a str, imports: HashMap<String, String>, source_url: &'a str) -> Self {
            Self {
        ...
        fn transform_expression(&self, expr: o::Expression) -> o::Expression {
            match expr {
                ...
                o::Expression::InvokeFn(mut e) => {
                    ...
                    e.args = e.args.into_iter().map(|arg| self.transform_expression(arg)).collect();
                ...
                o::Expression::LiteralArray(mut e) => {
                    e.entries = e.entries.into_iter().map(|entry| self.transform_expression(entry)).collect();
                ...
                o::Expression::ArrowFn(mut e) => {
                    match e.body {
                        o::ArrowFunctionBody::Expression(expr) => {
                            e.body = o::ArrowFunctionBody::Expression(Box::new(self.transform_expression(*expr)));
                        },
                        o::ArrowFunctionBody::Statements(stmts) => {
                            e.body = o::ArrowFunctionBody::Statements(self.transform_statements(stmts));
                        }
                    }
                ...
                o::Expression::BinaryOp(mut e) => {
                    ...
                    let is_assignment = matches!(e.operator,
                ...
                o::Expression::CommaExpr(mut e) => {
                    e.parts = e.parts.into_iter().map(|p| self.transform_expression(p)).collect();
        ...
        fn transform_statements(&self, stmts: Vec<o::Statement>) -> Vec<o::Statement> {
            stmts.into_iter().map(|stmt| self.transform_statement(stmt)).collect()
        }
            ...
            match expr {
                ...
                ast::Expression::ArrayExpression(e) => {
                    for el in &e.elements {
                        match el {
                            ast::ArrayExpressionElement::SpreadElement(s) => self.visit_expression(&s.argument),
                            target => {
                        ...
                        if let Some(decorators_arg) = expr.arguments[0].as_expression() {
                             if let Expression::ArrayExpression(decorators_array) = decorators_arg {
                                 ...
                                 for el in &decorators_array.elements {
                                     if let ast::ArrayExpressionElement::CallExpression(decorator_call) = el {
                                         // Check decorator name
                                         let mut dec_name = None;
                                         if let Expression::Identifier(ident) = &decorator_call.callee {
                                         ...
                                         if let Some(d_name) = dec_name {
                                             ...
                                             if self.selector.supports_declaration(d_name) && 
                                                (d_name == "Component" || d_name == "Directive" || d_name == "Pipe" || d_name == "Injectable" || d_name == "NgModule") {
                                                 
                                                 if decorator_call.arguments.len() > 0 {
                                                     if let Some(meta_arg) = decorator_call.arguments[0].as_expression() {
                                                          // Link!
                                                          let arg_expr_a: &'a ast::Expression<'a> = unsafe { std::mem::transmute(meta_arg) };
                                                          let oxc_node = OxcNode::Expression(arg_expr_a);
                                                          let value = AstValue::new(oxc_node, &self.host);
                                                          
                                                          match value.get_object() {
                                                             Ok(obj) => {
                                                                 let linker = self.selector.get_linker(d_name, "0.0.0", "0.0.0");
                                                                 let mut constant_pool = ConstantPool::new(false);
                                                                 
                                                                 // Link partial declaration (reads templateUrl!)
                                                                 let result_expr = linker.link_partial_declaration(&mut constant_pool, &obj, self.source_url, "0.0.0", Some(target_name));
                                                                 ...
                                                                 let mut assignment = format!("; {}.{} = {};", target_name, field_name, js_code);
...
println!("LOG: Visiting Component decorator");
                                                                      ...
                                                                      let d_span = decorator_call.span;
                                                                      self.replacements.push((d_span.start, d_span.end, "void 0".to_string()));
                                                                 }
                                                                 println!("[Rust Linker] Linked Decorator {} on '{}' -> {}", d_name, target_name, field_name);
                                                                 
                                                                 // Append after __decorate call
                                                                 let span = expr.span;
                                                                 self.replacements.push((span.end, span.end, assignment));
                                                             },
                                                             Err(e) => {
                ...
                if n.starts_with("ɵɵngDeclare") && self.selector.supports_declaration(n) {
                    ...
                    if expr.arguments.len() > 0 {
                        ...
                        if let Some(arg_expr) = expr.arguments[0].as_expression() {
                             ...
                             let arg_expr_a: &'a ast::Expression<'a> = unsafe { std::mem::transmute(arg_expr) };
                             let oxc_node = OxcNode::Expression(arg_expr_a);
                             let value = AstValue::new(oxc_node, &self.host);
                             match value.get_object() {
                                 Ok(obj) => {
                                     ...
                                     // Link!
                                     // Link!
                                     let result_expr = linker.link_partial_declaration(&mut constant_pool, &obj, self.source_url, "0.0.0", None);
                                 ...
                                 Err(e) => {
                                     self.errors.push(format!("Failed to parse metadata object: {}", e));
                                 }
                             }
                        }
                    }
    ...
    use std::io::Write;
    let mut log_file = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/linker.log").unwrap();
    ...
    if !visitor.errors.is_empty() {
        ...
        return Err(Error::new(Status::GenericFailure, visitor.errors.join("\n")));
    }
    
    writeln!(log_file, "Replacements count: {}", visitor.replacements.len()).unwrap();
