...
pub fn extract_metadata_from_linked(
    ...
    let ng_module_pattern = regex::Regex::new(
        r"(\w+)\.ɵmod\s*=\s*[^(]+\(\{[^}]*exports:\s*\[([^\]]*)\]"
    ).ok();
    ...
    let host_attrs_pattern = regex::Regex::new(
        r#"hostAttrs:\s*\[([^\]]*)\]"#
    ).ok();
    ...
    let directive_name_pattern = regex::Regex::new(
        r#"(\w+)\.(ɵcmp|ɵdir)\s*="#
    ).ok();
                ...
                if let Some(selectors_pos) = search_range.find("selectors:") {
                    ...
                    if let Some(open_pos) = after_selectors.find('[') {
                        ...
                        if found_closing {
                            ...
                            let host_attrs = extract_host_attrs_for_class(&name, linked_code, &host_attrs_pattern);
    ...
    if !modules.is_empty() || !directives.is_empty() {
        if let Ok(mut cache) = get_metadata_cache().write() {
            for module in &modules {
                cache.modules.insert(format!("{}:{}", module_path, module.name), module.clone());
            }
            for directive in &directives {
                cache.directives.insert(format!("{}:{}", module_path, directive.name), directive.clone());
            }
        }
    }
...
pub fn get_module_exports(module_path: &str, module_name: &str) -> Option<Vec<ExtractedDirective>> {
    ...
    let module = cache.modules.get(&format!("{}:{}", module_path, module_name))?;
    
    let mut result = Vec::new();
    for export_name in &module.exports {
        if let Some(directive) = cache.directives.get(&format!("{}:{}", module_path, export_name)) {
...
pub fn get_directive_metadata(module_path: &str, directive_name: &str) -> Option<ExtractedDirective> {
    let cache = get_metadata_cache().read().ok()?;
    cache.directives.get(&format!("{}:{}", module_path, directive_name)).cloned()
}
...
mod tests {
    ...
    fn test_parse_selectors_array() {
        ...
        assert_eq!(
            parse_selectors_array(r#"["button", "matButton", ""], ["a", "matButton", ""], ["button", "mat-button", ""]"#),
